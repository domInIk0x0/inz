...
# Define the Asymmetric Loss
class AsymmetricLoss(nn.Module):
    def __init__(self, alpha=2.0, beta=1.0):
        """
        :param alpha: Penalty for false negatives
        :param beta: Penalty for false positives
        """
    ...
    def forward(self, predictions, targets):
        ...
        positives = (targets == 1).float()
        negatives = (targets == 0).float()

        # Calculate loss for positive and negative classes
        loss_pos = -self.alpha * positives * torch.log(predictions[:, 1] + 1e-6)
        loss_neg = -self.beta * negatives * torch.log(1 - predictions[:, 1] + 1e-6)
        return (loss_pos + loss_neg).mean()


# Block definition with Dropout after BatchNorm
...
criterion = AsymmetricLoss(alpha=2.0, beta=1.0)

PATH = '/mnt/ip105/dpietrzak/panda/model/resnet18_hel_vs_cancer_asymetricloss.pth'
